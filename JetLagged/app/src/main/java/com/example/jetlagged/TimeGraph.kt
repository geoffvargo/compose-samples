...
@Composable
fun TimeGraph(hoursHeader: @Composable () -> Unit,
              dayItemsCount: Int,
	...
	Layout(contents = listOf(hoursHeader, dayLabels, bars),
		...
		val barPlaceables = barMeasureables.map { measurable ->
			...
			val barPlaceable = measurable.measure(constraints.copy(minWidth = barWidth, maxWidth = barWidth))
...
object TimeGraphScope {
	@Stable
	fun Modifier.timeGraphBar(start: LocalDateTime,
	                          end: LocalDateTime,
	                          hours: List<Int>): Modifier {
		val earliestTime = LocalTime.of(hours.first(), 0)
		val durationInHours = ChronoUnit.MINUTES.between(start, end) / 60f
		val durationFromEarliestToStartInHours = ChronoUnit.MINUTES.between(earliestTime, start.toLocalTime()) / 60f
		...
		return then(TimeGraphParentData(duration = durationInHours / hours.size, offset = offsetInHours / hours.size))
	}
}

class TimeGraphParentData(val duration: Float,
                          val offset: Float) : ParentDataModifier {
